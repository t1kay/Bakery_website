...
public class RAGService {
    ...
    public String getAnswer(String question) {
        ...
        try {
            ...
            String requestBody = objectMapper.writeValueAsString(Map.of("inputs", ragPrompt, "parameters",
                    Map.of("max_new_tokens", 100, "return_full_text", false)));
            ...
            if (huggingfaceApiToken != null && !huggingfaceApiToken.isBlank()) {
                String authHeader = "Bearer " + huggingfaceApiToken.trim();
                reqBuilder.header("Authorization", authHeader);
            } else {
                System.err.println("Hugging Face API token not configured (property: huggingface.api.token)");
                return "AI token chưa được cấu hình trên server.";
            }
